            ...
            <div class="flex flex-col items-center justify-center">
              <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                  ...
                  <div class="mb-4">
                    <div>
                      <label for="email" class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">ログインID</label>
                      <Field name="email" rules="required|max:255|email" v-model="state.model.email" placeholder="ログインID" v-slot="{ field, handleChange }">
                        <InputText v-model="state.model.email" v-bind="field" @update:model-value="handleChange" class="w-full md:w-[30rem]" placeholder="ログインID" />
                      </Field>
                    </div>
                    <ErrorMessage class="p-error" name="email" />
                  </div>
                  <div class="mb-4">
                    <label for="password" class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">パスワード</label>
                    <Field name="password" rules="required|max:16|min:10|password_rule" v-model="state.model.password" v-slot="{ field, handleChange }">
                      <Password
                        v-bind="field"
                  ...
                  <div class="flex items-center justify-between mt-2 mb-8 gap-8">
                    <div class="flex items-center">
                      <Checkbox inputId="rememberMe" v-model="state.model.remember_me" name="remember_me" value="1" />
                      <label for="rememberMe" class="ml-2"> 次回から自動的にログイン </label>
                    </div>
                  </div>
